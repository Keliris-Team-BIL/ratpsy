...
for rat=[7,8,11,19]
    ...
    N_post=gk_get_psychometric(linPost(linPost.ratNumber==rat,:));
    N_postLog=gk_get_psychometric(logPost(logPost.ratNumber==rat,:));
    figure(rat); hold on;
    display('-------PRE---------');
    gk_PAL_PFML(N_pre,[1.0, 0.2, 0.2],1);
    grid on;
    display('-------POST--------');
    gk_PAL_PFML(N_post,[0.2, 0.2, 1.0],1);
    display('-------LOG---------');
    gk_PAL_PFML(N_postLog,[0.2, 1.0, 0.2],1);
